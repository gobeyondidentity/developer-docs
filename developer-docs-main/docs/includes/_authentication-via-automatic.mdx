...
Create a `bi-authenticate.tsx` page under `/next-auth-example/pages`. As long as your `Invoke URL` is configured properly in your [Authenticator Config](/docs/authentication), this is the page that will be redirected to during an authorization flow. Copy the following code snippet into that page.
...
3. A `redirectURL` is returned from a successful `AuthenticateResponse`. The authorization code and the state parameter are attached to this URL. You can now exchange the code for an id token using your [Beyond Identity Token Endpoint](../authentication/embedded-sdk-add-authentication.mdx#token-exchange).
...
3. A `redirectURL` is returned from a successful `AuthenticateResponse`. The authorization code and the state parameter are attached to this URL. You can now exchange the code for an id token using your [Beyond Identity Token Endpoint](/docs/embedded-sdk-add-authentication#token-exchange).
