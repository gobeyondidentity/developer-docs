...
- Before calling [`EmbeddedSdk.authenticate()`](/docs/sdk-setup?sdks=flutter#authentication), you must Authorize with Okta. 
...
:::note
At this time, there is no Okta SDK for Flutter, so we recommend [using the Web](/docs/okta-oidc#using-the-web).
:::
