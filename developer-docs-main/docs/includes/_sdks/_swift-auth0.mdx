...
- Before calling [`Embedded.shared.authenticate`](/docs/sdk-setup?sdks=swift#authentication), you must Authorize with Auth0. 
...
:::note
At this time, the authorization flow cannot be completed using the SDK, so we recommend [using the Web](/docs/auth0#using-the-web). 
:::
