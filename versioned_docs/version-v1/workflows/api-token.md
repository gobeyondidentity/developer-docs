...
1. [Beyond Identity Admin Console](#create-token-with-the-beyond-identity-admin-console)
2. [Beyond Identity APIs](#create-token-with-the-beyond-identity-api)
...
There are two token formats that can be validated: `self-contained` or `referential`. When validating self-contained tokens, consider if you want to revoke your tokens. If revoked tokens are accepted (as in a MVP case), then the token can be validated [offline](#offline-validation) by validating the signature and parsing the claims of the JWT token. In all other cases the token should be [introspected](#introspection-online-validation).
