...
Configure your application's basic settings. After saving, the values entered will be used to generate OIDC configuration endpoints for your application as well as your Client ID and Client Secret.
  ...
  |  **Resource Server**  |  If the app is linked to a resource server, the resource server determines the [scopes the app can be allowed](../api-tokens/api-scopes) to issue.   |
...
Select the [authenticator configuration type](../foundations/authenticator-config).
...
1. Create a [Resource Server](../foundations/platform-overview#resource-server). A Resource Server is a namespace for application scopes that are a set of all scopes supported by the application. 
