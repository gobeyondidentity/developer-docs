...
<Row>
  <Col span={12}>
    <Alert message="I've added some clarifications here to support my other articles (jen)" type="info" />
  </Col>
</Row>
...
Configure your application's basic settings. After saving, the values entered will be used to generate OIDC configuration endpoints for your application as well as your Client ID and Client Secret.
  ...
  | --- | --- |
  | **Protocol**   |  <mark>Why choose one over the other?</mark>For examples of how to use OAuth and OIDC flows to request tokens, see [request api tokens](../api-tokens/create-api-token.md#token-request-scenarios)<ul><li><b>OAuth2</b>: With the OAuth2 protocol, you can use either the client credentials grant type or the authorization code grant type.</li><li><b>OIDC</b>: With the OIDC protocol, you must use the authorization code grant type. With OIDC, you can obtain an id_token as well as an access_token. </li></ul>  | 
  ...
  |  **Resource Server**  |  If the app is linked to a resource server, the resource server determines the [scopes the app can be allowed](../api-tokens/api-scopes.md) to issue.   |
...
Select the [authenticator configuration type](../foundations/authenticator-config.mdx).
...
Before creating an application by API, you must have an [API access token](../api-tokens/create-api-token.md#api). You'll also need to create a Resource Server and an Authenticator Config, which you'll do below. A Resource Server is a namespace for application scopes that are a set of all scopes supported by the application. An Authenticator Config defines the invocation type, invoke URL, and trusted origins. 

1. Create a [Resource Server](../foundations/platform-overview.md#resource-server). A Resource Server is a namespace for application scopes that are a set of all scopes supported by the application. 
