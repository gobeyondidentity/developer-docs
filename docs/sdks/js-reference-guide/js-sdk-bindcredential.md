...
To obtain a passkey binding URL for an existing identity, replace Step 1 above with an API call such as the one below to retrieve the identity ID for a known user. Once you have the identity ID, the process to retrieve the passkey binding URL is identical to Step 2 above.
...
The following example shows how to obtain a passkey binding URL via email for a known identity ID.
