...
Now that your client has recieved an **authorization code** it's time to exchange the code for an **access token** and **id token**. An access token is used to access protected resources on behalf of a user. The id token contains information about the user and can be used to prove that the user has been authenticated.
