...
1. **Authentication:** When a user logs in with valid credentials, the server issues either an access token or an access token and a refresh token, depending on whether you have enabled refresh tokens for your application in the [Admin Console](/docs/next/session-management-console#refresh-token-configuration-refresh-token-setting).

1. **Accessing Protected Resources:** The client includes the access token in the request headers whenever accessing protected resources on the server. You can use [Custom Claims](/docs/next/create-api-token#example-create-a-token-with-custom-claims) to further protect resources on the server by adding metadata to a token, such as a custom user role or a user's subscription or vip status.
