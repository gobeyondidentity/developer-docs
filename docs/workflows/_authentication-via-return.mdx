...
When Your [Authenticator Config](../platform-overview/authenticator-config)'s Invocation Type is set to `Manual` the OAuth2 challenge will be returned directly as part of a JSON response. The result to your end user is a more seamless authentication without a webview. Make a GET request to your [crafted authorization URL](authentication#craft-authorization-url) and automatically recieve an `authenticate_url` that you can pass to the Embedded SDK's `authenticate` function. You can confirm the validity of the URL with `isAuthenticateUrl`.
