...
Once you have the incoming url, pass it into the SDK to complete the binding process. Upon success a private key will have been created in the device's hardware trust module, and public key will have been sent to the Beyond Identity Cloud. At this point the user has a passkey enrolled on this device. Don't forget to initalize your SDK ahead of time. For more information see [SDK setup](./sdk-setup)
