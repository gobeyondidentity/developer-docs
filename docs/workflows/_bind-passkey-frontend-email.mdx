...
Once you have the incoming url, pass it into the SDK to complete the binding process. This will embed the passkey into the device or broswer that the user is currently on. Don't forget to initalize your SDK ahead of time. For more information see [SDK setup](docs/workflows/sdk-setup)
