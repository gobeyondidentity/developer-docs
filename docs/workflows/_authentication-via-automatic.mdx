...
When Your [Authenticator Config](../platform-overview/authenticator-config)'s Invocation Type is set to `Automatic`, a successful authorization will redirect the user back to your application specified by Authenticator Config's `Invoke URL`. Make sure to set up deep linking in your native applications with either an app scheme or a Universal URL / App link.
...
Make sure to present some logic to the user to select a passkey to authenticate with. Don't forget to initalize your SDK ahead of time. For more information see [SDK Setup](./sdk-setup).
...
1. Build a [CustomTabsIntent](https://developer.android.com/reference/androidx/browser/customtabs/CustomTabsIntent), and launch your [crafted authorization URL](authentication#craft-authorization-url).
...
3. A `redirectURL` is returned from a successful `AuthenticateResponse`. The authorization code and the state parameter are attached to this URL. You can now exchange the code for an id token using your [Beyond Identity Token Endpoint](authentication#token-exchange).
...
1. Start an `ASWebAuthenticationSession`, and launch your [crafted authorization URL](authentication#craft-authorization-url).
...
1. Subscribe to the `useAuthRequest` hook with the Beyond Identity discovery url. Many of the values used to configure the request are the values from your [crafted authorization URL](authentication#craft-authorization-url).
...
1. Check that an `InAppBrowser` isAvailable, and launch your [crafted authorization URL](authentication#craft-authorization-url).
...
1. Make a call to `FlutterWebAuth.authenticate()`, and launch your [crafted authorization URL](authentication#craft-authorization-url).
