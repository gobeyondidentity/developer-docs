...
Inside the **package.json** file, ensure that the scripts that run the app use the correct port that matches the app's configured redirect URI in the BI tenant. For example, if the redirect URI is http://localhost:8083/api/auth/callback/beyondidentity, the script should specify port '8083' using the -p option, such as `"next -p 8083"`  
...
3. Enter a name, username, and email address. Then be sure to enter an **Email** address that is a real address where you can receive email.  
...
3. Select the app and then select **Delivery method** 'Send an email to user'.  
