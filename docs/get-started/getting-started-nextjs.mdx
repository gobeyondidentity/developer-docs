...
In VS Code, execute `npm run dev` at the terminal prompt. From the browser where you created the passkey above, navigate to `http://localhost:3000`. Here you should be able to click Sign in and sign in to the app as shown below. Registration and passkey binding will happen in the Hosted Web UI. On the first authentication, a user will be prompted to provide an email for verification. The user will then have the option to generate a passkey on trusted devices. All following authentications may be used with a passkey.

![NextAuth.js Example Home Page](../images/js-sdk-example-app-signin-1.png 'NextAuth.js Example Home Page')

![Register Beyond Identity](../images/hosted-web-email.png 'NextAuth.js Register Beyond Identity')

![Register Beyond Identity](../images/hosted-web-passkey.png 'NextAuth.js Passkey Beyond Identity')
