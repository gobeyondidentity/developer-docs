...
3. Save your changes, but don't close the file because you'll use it to add the TENANT_ID, API_TOKEN, REALM_ID, and other variables to complete the setup.  
...
5. In the `.env.local` config file, paste the `API_TOKEN`. Then save your changes but don't close it becuase you'll need it for the next step.
...
5. Copy the **Realm Id** value and paste it into the `.env.local` file in `REALM_ID`.  Then your changes but don't close it yet because you're not done.
...
6. Scroll down and set the **Invoke URL** and **Trusted Origins** to the following localhost and port. For more details about these settings, see about [Authenticator Types](../foundations/authenticator-config).
