All [Beyond Identity API](../foundations/api-endpoints.mdx) endpoints require an access token for authentication. The access token is generated through OAuth 2.0 or OIDC, using either the **Authorization Code** or **Client Credentials** flow. 
...
You can create an API access token using the [Beyond Identity Admin Console](../api-tokens/create-api-token.md#admin-console) or the [Beyond Identity APIs](../api-tokens/create-api-token.md#api). You'll generate the access token with your **Beyond Identity Management API** application to authorize access to any Beyond Identity APIs. The Beyond Identity Management API application is provided by default upon tenant creation and resides within the **Beyond Identity Admin Realm**. 
