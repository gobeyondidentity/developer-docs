All [Beyond Identity API](/api) endpoints require an access token for authentication. The access token is generated through OAuth 2.0 or OIDC, using either the **Authorization Code** or **Client Credentials** flow. 
