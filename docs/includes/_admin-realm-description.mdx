...
:::note
When a new realm is created, the SCIM server is available by default and a separate SCIM server is available for each realm. However, to use any SCIM functionality, an access token with `scim:all` scope is required. See [SCIM server setup](/docs/next/scim-server) for more details.
:::
