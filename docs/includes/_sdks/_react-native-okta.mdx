...
- [React Native SDK Setup](../sdks/sdk-setup.mdx?sdks=reactnative)

- Before calling [`Embedded.authenticate`](../sdks/sdk-setup.mdx?sdks=reactnative#authentication), you must Authorize with Okta. 
...
:::note
At this time, there is no way to intercept the first URL to authenticate with Beyond Identity, so we recommend [using the Web](#using-the-web). 
:::
