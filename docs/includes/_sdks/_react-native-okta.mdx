...
- Before calling [`Embedded.authenticate`](/docs/next/sdk-setup?sdks=reactnative#authentication), you must Authorize with Okta. 
...
:::note
At this time, there is no way to intercept the first URL to authenticate with Beyond Identity, so we recommend [using the Web](/docs/next/okta-oidc#using-the-web). 
:::
