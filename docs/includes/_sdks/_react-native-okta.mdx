...
- [React Native SDK Setup](/docs/sdk-setup?sdks=reactnative)

- Before calling [`Embedded.authenticate`](/docs/sdk-setup?sdks=reactnative#authentication), you must Authorize with Okta. 
...
:::note
At this time, there is no way to intercept the first URL to authenticate with Beyond Identity, so we recommend [using the Web](/docs/okta-oidc#using-the-web). 
:::
...
Make sure the [Authenticator Config](/docs/authentication#embedded-sdk) in the Beyond Identity Console is set to type `Embedded` and that the Invoke URL points to your application with either an App Scheme or a Universal Link.
