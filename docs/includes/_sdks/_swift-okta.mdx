...
- Before calling [`Embedded.shared.authenticate`](/docs/next/sdk-setup?sdks=swift#authentication), you must Authorize with Okta. 
...
:::note
At this time, the authorization flow cannot be completed using the SDK, so we recommend [using the Web](/docs/next/okta-oidc#using-the-web). 
:::
