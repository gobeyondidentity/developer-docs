...
- [Flutter SDK Setup](../sdks/sdk-setup.mdx?sdks=flutter)   

- Before calling [`EmbeddedSdk.authenticate()`](../sdks/sdk-setup.mdx?sdks=flutter#authentication), you must Authorize with Okta. 
...
:::note
At this time, there is no Okta SDK for Flutter, so we recommend [using the Web](#using-the-web).
:::
...
The result will be a URL with the Invoke URL scheme. You can call [`EmbeddedSdk.authenticate()`](../sdks/sdk-setup.mdx?sdks=flutter#authentication), using the result. You can confirm the validity of the URL with [`EmbeddedSdk.isAuthenticateUrl()`](../sdks/sdk-setup.mdx?sdks=flutter#authenticate-url-validation).
