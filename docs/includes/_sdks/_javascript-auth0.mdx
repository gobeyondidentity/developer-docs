### Prerequisites
 ...
 - [JavaScript SDK Setup](../sdks/sdk-setup.mdx?sdks=javascript)


:::tip NextAuth.js
This part of the integration guide uses [NextAuth](https://next-auth.js.org/) for all OAuth/OIDC flows. All code snippets are provided in the context of the [NextAuth Example App](https://github.com/nextauthjs/next-auth-example).
:::
...
Note that you'll need to fill in the `<AUTH0_ID>`, `<AUTH0_SECRET>`, and `<AUTH0_ISSUER>` with the values you generated when creating your application in Auth0. See the [following](/auth0) guide mentioned in the [prerequisites](#prerequisites).

<br />

### Wiring up [`embedded.authenticate`](../sdks/sdk-setup.mdx#authentication)
...
1. The `useEffect` is only called once on page load. In this function, we initialize the Beyond Identity SDK and use [`embedded.isAuthenticateUrl`](../sdks/sdk-setup.mdx?sdks=javascript#authenticate-url-validation) to check if the current page that was redirected to is in fact a valid `bi-authenticate` URL.
