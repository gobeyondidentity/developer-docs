### Prerequisites
 ...
 - [JavaScript SDK Setup](/sdk-setup?sdks=javascript)
...
### Wiring up [`embedded.authenticate`](/sdk-setup#authentication)
...
1. The `useEffect` is only called once on page load. In this function, we initialize the Beyond Identity SDK and use [`embedded.isAuthenticateUrl`](/sdk-setup?sdks=javascript#authenticate-url-validation) to check if the current page that was redirected to is in fact a valid `bi-authenticate` URL.
