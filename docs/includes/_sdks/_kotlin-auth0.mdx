...
- Before calling [`EmbeddedSdk.authenticate()`](/docs/sdk-setup?sdks=kotlin#authentication), you must Authorize with Auth0. 
...
:::note
At this time, the authorization flow cannot be completed using the SDK, so we recommend [using the Web](/docs/auth0#using-the-web). The SDK is not designed to work with a secondary IdP. The token exchange, however, can be completed using the SDK.
:::
