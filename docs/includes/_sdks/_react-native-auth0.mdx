...
- Before calling [`Embedded.authenticate`](/docs/sdk-setup?sdks=reactnative#authentication), you must Authorize with Auth0. 
...
:::note
At this time, there is no way to intercept the first URL to authenticate with Beyond Identity, so we recommend [using the Web](/docs/auth0#using-the-web). 
:::
