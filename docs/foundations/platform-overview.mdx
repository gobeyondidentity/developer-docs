...
<img
  id="diagrams"
  width="70%"
  src="https://developer.beyondidentity.com/assets/images/account-setup-diagram-4c40ff55e0f3de39e87e3a82d801ed37.png"
/>
...
A resource server is a logical grouping of [scopes](/docs/api-scopes). Scopes are data elements included in [access tokens](/docs/api-token-overview) that enable applications to provide fine grained levels of access control.
...
In order to use scopes for your application's tokens, you must [create at least one resource server](/docs/add-resource-server) in the same realm as the application that requires scopes.
...
For more details, see [Authenticator Types](/docs/authentication).
...
The **Admin Console Access** User Group is created, and the identity established during provisioning is added. Adding users to the group will allow them to log into the Console as Administrators of your tenant utilizing any credentials established within the Admin Realm.
