...
id: authenticator-config
slug: '/authenticator-config'
description: 'An authenticator config is used to tell us how and where to invoke passkey creation and authentication flows for your native/mobile or web application. Each application is tied to a single authenticator type, which is configured in Authenticator Config. This article will help you understand each of the authenticator type options and how they impact Beyond Identity passkey flows.'
keywords: 
 - deployment model
 - embedded sdk
...
The Hosted Web Authenticator is a constrained version of the Embedded SDK authenticator type. It requires less configuration and code, but it does require redirecting your users to a web-based authenticator hosted by Beyond Identity to utilize your application's passkeys on a user's device. The Hosted Web Authenticator is a Beyond Identity hosted page that embeds the Javascript SDK. Think of this as a wallet hosted by Beyond Identity Cloud that holds an identity's passkeys.


Hosted Web Authenticator is a constrained version of the Embedded Authenticator Config with the following default values:

<ul><li><b>Invocation Type</b>: automatic </li><li><b>Invoke URL</b>: https://auth-&#123;REGION&#125;.beyondidentity.com/authenticator/</li><li><b>Trusted Origin</b>: [https://auth-&#123;REGION&#125;.beyondidentity.com/authenticator/]</li></ul>


:::tip KEEP IN MIND
Passkeys are tied to an origin and they are stored on a specific browser.
:::
...
<Card class="intro-card">
      <a href="https://gobeyondidentity.github.io/bi-sdk-android/">
            ...
            <path d="M32 32H0V0h32L16 16l16 16Z" fill="url(#KotlinLogo_svg__a)"></path>
            <defs>
...
<Card class="intro-card">
            ...
            <svg width="32" height="32" viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg" {...props}>
                  <path xmlns="http://www.w3.org/2000/svg" d="M16.1435 65.9891L0.744629 50.5902L50.7446 0.594238H81.5384L16.1435 65.9891ZM81.5384 46.7304H50.7446L39.2096 58.2654L54.6085 73.6643" fill="#42A5F5" fill-opacity="0.8"/>
                  <path xmlns="http://www.w3.org/2000/svg" d="M39.2095 89.0592L50.7445 100.594H81.5384L54.6084 73.6643" fill="#0D47A1"/>
                  <path xmlns="http://www.w3.org/2000/svg" d="M23.8472 73.6724L39.2218 58.2937L54.5965 73.6684L39.2218 89.0471L23.8472 73.6724Z" fill="#42A5F5"/>
                  <path xmlns="http://www.w3.org/2000/svg" d="M39.2217 89.047L54.5963 73.6724L56.7425 75.8185L41.3678 91.1932L39.2217 89.047Z" fill="url(#paint0_linear_25_430)"/>
                  <path xmlns="http://www.w3.org/2000/svg" d="M39.2095 89.0591L62.0573 81.1656L54.6084 73.6602" fill="url(#paint1_linear_25_430)"/>
            </svg>
            <p><b>Flutter</b></p>
            </a>
      </Card>
...
:::note IMPORTANT
The **Invocation Type** only impacts the authentication flow. It has no effect on the [passkey creation/binding flow](../workflows/passkey-creation.md).
:::
...
The invoke URL is a single URL that _points_ to where your application is. In the case of a native application (iOS, Android, Flutter, React Native), this is either an App Scheme or a Universal URL / App Link. In the case of a web application, this is just a URL to your web application or a specific page of your web application.
...
There are two scenarios in which the Invoke URL is used for:

<mark>If I understand the documentation correctly, the following are scenarios. If I'm wrong, then we just need to change the column headers and give details and relevance. In other words, "what is it" and "why" should the customer care.</mark>

| Scenario | When to use |
| --- | --- |
| **Binding a passkey**  | You want to generate a passkey link to deliver to your users how you want (inline, SMS, email, etc.) or you want to send your users an email with a link to create their passkey. Passkeys are created through binding jobs where a binding link gets generated to bind a passkey to a specific device or browser.   |
| **Authentication**  |   |
...
<mark>I think the following can be an _include file as it's also part of the "workflow" (Passkey Binding) and "how-to" (Bind passkeys) topics.</mark>

<Tabs>

<TabItem value="EMAIL" label="EMAIL delivery_method">

<BindEmailDiagram />

</TabItem>

<TabItem value="RETURN" label="RETURN delivery_method">
...
```javascript title="Invocation Type Automatic"
HTTP/1.1 302 Found
Location: $invoke_url/bi-authenticate?request=<request>
```


```javascript title="Invocation Type Manual"
HTTP/1.1 200 OK
