...
**ID Tokens** come from [OIDC](/docs/terms/oidc.md) and are used to prove that the user has been authenticated. [ID Tokens](/docs/terms/id-token.md) are always [JSON Web Tokens (JWT)](/docs/terms/jwt.md). When the payload of the JWT is inspected you will find claims, information about the user such as the user's name and email and the OIDC service. You can use this token to determine if the user is logged in.
...
:::tip
Other parameters such as **state**, **code_challenge_method** and **code_challenge** may be set for higher security, but the above are the minimum required.
:::

| Parameter         | Value                                                                                            |
| ----------------- | ------------------------------------------------------------------------------------------------ |
| **response_type** | Set this to `code` to specify the Authorization Code grant type                                  |
| **scope**         | Set this to `oidc` to specify an OIDC request.                                                   |
...
:::tip
If you are using the Beyond Identity **[Hosted Web Authenticator](/docs/authentication)**, all you need to do is set up a button for your user with the authorization URL to start the sign in flow.
:::
